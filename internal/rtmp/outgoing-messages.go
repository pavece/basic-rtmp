...
func sendWindowAckSize(connection net.Conn, window uint32) {
	var body []byte
	body = binary.BigEndian.AppendUint32(body, window)
	chunks := buildMessageChunks(body, 2, 5, 0)
...
func sendPeerBandwidth(connection net.Conn, window uint32, limitType uint8) {
	var body []byte
	body = binary.BigEndian.AppendUint32(body, window)
	body = append(body, byte(limitType))

	chunks := buildMessageChunks(body, 2, 6, 0)
	for _, chunk := range(chunks) {
		connection.Write(chunk)
	}
}
...
func sendConnectionResultCommand(connection net.Conn, transactionId int){
